...
def _outlook_entry_available(entry: dict[str, Any] | None) -> bool:
    ...
    if current == "in_use":
        ...
        try:
            ...
            age = (datetime.now(timezone.utc) - current_ts).total_seconds()
    ...
    with _outlook_token_state_lock:
        ...
        if str(scope) == "failed":
            remove = {
                ...
                if str(value.get("state") or "") in {"failed", "token_invalid", "in_use"}
            }
            for key in remove:
                store.pop(key, None)
            _save_outlook_token_state(store)
...
class OutlookTokenError(RuntimeError):
    """refresh_token failed, usually because the mailbox credential is invalid."""


def _clean_outlook_value(value: object) -> str:
    ...
    def create_mailbox(self, username: str | None = None) -> dict[str, Any]:
        ...
        with _outlook_token_state_lock:
            ...
            if credential is None:
                raise RuntimeError(
                    f"[{self.label}] OutlookToken 邮箱池暂无可用邮箱（共 {len(self.pool)} 个，"
                    "已用尽或全部占用/失效），请导入新邮箱或重置池状态"
                )
